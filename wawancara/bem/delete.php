...
if (!empty($id)) {
    ...
    $querySelect = "SELECT foto_diri, ktm, follow_ig FROM bem WHERE id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $files = [
            ['filename' => $row['foto_diri'], 'folder' => '../../uploads/bem/foto_diri/'],
            ['filename' => $row['ktm'], 'folder' => '../../uploads/bem/ktm/'],
            ['filename' => $row['follow_ig'], 'folder' => '../../uploads/bem/follow_ig/'],
        ];
        ...
        $queryDelete = "DELETE FROM bem WHERE id = ?";
