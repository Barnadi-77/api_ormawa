...
function uploadFile($inputName, $targetFolder)
{
    ...
    $uploadDir = __DIR__ . "/../../uploads/icrt/$targetFolder/";
...
$subyt = uploadFile('subscribe_yt', 'subscribe_yt');
$folig = uploadFile('follow_ig', 'follow_ig');

// Cek apakah semua file berhasil diupload
if (!$foto['status'] || !$ktm['status'] || !$subyt['status'] || !$folig['status']) {
    $errorMessage = '';
    if (!$foto['status']) {
        ...
        $errorMessage = $subyt['message'];
    } elseif (!$folig['status']) {
        $errorMessage = $folig['message'];
    }
...
// Simpan ke database
$stmt = $conn->prepare("INSERT INTO pendaftar (nama, nim, email, kelas, angkatan, kontak, prodi, motivasi, foto_diri, ktm, subscribe_yt, follow_ig) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
