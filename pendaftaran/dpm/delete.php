...
if (!empty($id)) {
    ...
    $querySelect = "SELECT foto_diri, ktm, follow_ig FROM dpm WHERE id = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        $files = [
            ['filename' => $row['foto_diri'], 'folder' => '../../uploads/dpm/foto_diri/'],
            ['filename' => $row['ktm'], 'folder' => '../../uploads/dpm/ktm/'],
            ['filename' => $row['follow_ig'], 'folder' => '../../uploads/dpm/follow_ig/'],
        ];
        ...
        $queryDelete = "DELETE FROM dpm WHERE id = ?";
